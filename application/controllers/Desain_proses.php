...
class Desain_proses extends CI_Controller {
    ...
    function __construct()
    {
        ...
        $this->load->model(array('Tbl_desain_checkout_model','Tbl_desain_model','KirimWa_model','Tbl_user_model'));
    ...
    public function index() {
        ...
        //     // ALL group kota
            $sql = "select * from tbl_desain group by group_desain";  // == 0 atau < 1---> Kirimaya ( INT = 11  , default = 0 )
            $data_desain_all_group = $this->db->query($sql)->result();

            $sql = "select * from tbl_desain";  // == 0 atau < 1---> Kirimaya ( INT = 11  , default = 0 )
            $data_desain_all = $this->db->query($sql)->result();
        ...
        $data = array(
            ...
            'data_desain_all_group' => $data_desain_all_group,
            'data_desain_all' => $data_desain_all,
     
        );

        // print_r($data['data_property_dijual_solo']);
   
        $this->load->view("DESAIN_PROSES/desain_list_pemilihan",$data);
    }
    ...
    public function Proses_beli_pilih($uuid_hunian=null) 
    {
        ...
        $data_desain       = $this->db->get('tbl_desain')->row();
        $id_hunian=$data_desain->id;
        ...
        $this->db->where('id_tbl_desain',$id_hunian);
        $data_desain_terpilih       = $this->db->get('tbl_desain_detail')->result();

        // print_r($data_desain_terpilih);
        // print_r("<br/>");
        // print_r("<br/>");

        $data = array(
            'data_desain_terpilih' => $data_desain_terpilih,
            'id_hunian' => $id_hunian,
        ...
        // print_r($data['data_desain_terpilih']);
   
        $this->load->view("DESAIN_PROSES/Proses_beli_pilih",$data);  
    }
    ...
    public function Checkout_action($uuid_hunian=null) {
        ...
        $data = array(
            'uuid_desain' => $uuid_hunian,
            'group_desain' => $data_desain->group_desain,
            'nama_desain' => $data_desain->nama_desain,
            'model' => $data_desain->model,
            'paket' => $data_desain->paket,
            'harga' => $data_desain->harga,
            // 'deskripsi' => $data_desain->deskripsi,
            // 'poto' => $data_desain->poto,
            // 'path_file' => $data_desain->path_file,
            'luas_tanah' => $data_desain->luas_tanah,
            'luas_bangunan' => $data_desain->luas_bangunan,
            'jumlah_kamar' => $data_desain->jumlah_kamar,
            'jumlah_kamar_mandi' => $data_desain->jumlah_kamar_mandi,
            'jumlah_lantai' => $data_desain->jumlah_lantai,
        ...
        // `id`, `uuid`, `id_users_pemilik`, `group_desain`, `nama_desain`, ``, ``, ``, ``, ``, ``, `luas_tanah`, `luas_bangunan`, `jumlah_kamar`, `jumlah_kamar_mandi`, ``
        ...
        redirect(site_url('Desain_proses/Proses_beli_pilih_checkout/'.$data_checkout->uuid_checkout));
    }
