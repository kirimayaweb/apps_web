...
class Gociko_time_run extends REST_Controller {
    ...
    function index_get() {
        ...
        if ($namatoko == '') {
            ...
            $data = $this->db->get('tbl_gocikotime')->result();
        } else {
            
            // CEK APAKAH ADA JAM RUNNING GOCIKO SESUAI NAMA TOKO DAN SETTING JAM
            $jam_sekarang = date("H");
            $jam_sekarang_plus_1 = date("H")+1;
            $get_menit_sekarang = date("i");
            $menit_sekarang_kurang_1 = date("i")-1;
            $menit_sekarang_plus_1 = date("i")+1;
            
            $sql = "SELECT * FROM tbl_gocikotime WHERE namatoko = '$namatoko' AND (jam >= $jam_sekarang AND jam < $jam_sekarang_plus_1  ) AND ( menit >= $menit_sekarang_kurang_1 AND menit <= $menit_sekarang_plus_1 ) ";
            $data_time=$this->db->query($sql)->result();

            // print_r($data_time);
            // print_r("<br/>");

            // print_r($data_time->idtoko);
            // print_r("<br/>");
            ...
            if($this->db->query($sql)->num_rows()>0){
                ...
                //RUNNING GOCIKO DAN SIMPAN KE HISTORY    
                    //kirim ke response ==> RUNNING GOCIKO
                        $data = array(
                            ...
                            'play_gociko'        => $this->db->query($sql)->row()->kode_mp3,
                            'play_time'        => $this->db->query($sql)->row()->playtime_mp3
                            );
                    ...
                    //simpan ke history
                        ...
                        // $this->db->insert('table_history_connected', $data_history);
                    
            }else{
                
                // TIDAK RUNNING GOCIKO PER TOKO , CEK KE TABEL RUNNING SEMUA: JIKA ADA TIME MAKA RUN GOCIKO

                $sql_ALL = "SELECT * FROM table_sys_jam_running WHERE (jam >= $jam_sekarang AND jam < $jam_sekarang_plus_1  ) AND ( menit >= $menit_sekarang_kurang_1 AND menit <= $menit_sekarang_plus_1 ) ";
                
                // $data_time=$this->db->query($sql)->result();
                
                if($this->db->query($sql_ALL)->num_rows()>0){
                    //RUNNING GOCIKO DARI DATA MP3 SEMUA TOKO
                ...
                // TIDAK RUNNING GOCIKO PER TOKO & TETAP SIMPAN KE HISTORY
                     
                     //SET DATA UNTUK TIDAK RUNNING GOCIKO
            ...
            //SIMPAN KE HISTORY ==> NAMA TOKO SUDAH KONEK KE SERVER ( RUNNING GOCIKO / RIDAK RUNNING )
